...
public class InternalApplicationIntegrationTest {
	...
	public void shouldCreateMessageSuccessfully() {
		...
		ResponseEntity<String> response = this.restTemplate.postForEntity("http://localhost:" + port + "/message",
	...
	public void shouldReturnAllMessages() {
		ResponseEntity<String> response = this.restTemplate.getForEntity("http://localhost:" + port + "/message",
				String.class);
	...
	public void shouldReturnMessageNotFound() {
		ResponseEntity<String> response = this.restTemplate.getForEntity("http://localhost:" + port + "/message/0",
				String.class);
	...
	public void shouldDeleteMessageSuccessfully() {
		...
		HttpEntity<String> request = new HttpEntity<>("{\"message\":\"hi\"}", headers);
		ResponseEntity<String> createResponse = this.restTemplate.postForEntity("http://localhost:" + port + "/message",
				request, String.class);

		this.restTemplate.delete("http://localhost:" + port + "/message/" + createResponse.getBody());

		ResponseEntity<String> response = this.restTemplate
				.getForEntity("http://localhost:" + port + "/message/" + createResponse.getBody(), String.class);
		assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
		assertTrue(response.getBody().contains("Message not found"));
	}
}
